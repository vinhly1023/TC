...
describe "CABO - Search SKU and check YMAL information on PDP page - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST.upcase}" do
  ...
  tc_num = 0
  atg_cabo_app_center_page = AtgAppCenterCaboEnglishPage.new
  ...
  titles_list.each do |title|
    ...
    e_ymal_info = atg_cabo_app_center_page.get_ymal_in_database(title_ymal, General::LOCALE_CONST, true)
    e_ymal = nil
    ...
    context "#{tc_num += 1}. SKU = '#{title_sku}' - #{title_short_name}" do
      ...
      context 'Search SKU and go to PDP page' do
        it 'Search by SKU' do
          ...
          # Get product info
          product_html = atg_cabo_app_center_page.catalog_html_content
          title_info = atg_cabo_app_center_page.catalog_product_info(product_html, title_prod_number)
        end
        ...
        it 'Get YMAL information on PDP page' do
          a_ymal_info = atg_cabo_app_center_page.ymal_info_on_pdp
        end
          ...
          context "#{index + 1} - SKU = '#{e[:sku]}' - Title = '#{e[:title]}'" do
            ...
            it 'Verify app is available in locale' do
              if a_ymal.nil?
                ...
                expect(atg_cabo_app_center_page.support_locale?(e_ymal, General::LOCALE_CONST)).to eq(true)
              end
            end
          end
        end
