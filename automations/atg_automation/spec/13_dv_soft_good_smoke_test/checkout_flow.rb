...
def dv_checkout_flow(db_url, email, platform = nil)
  # Account information
  ...
  # ATG page information
  env = General::ENV_CONST
  locale = General::LOCALE_CONST
  url = Title.url_mapping db_url
  ...
  feature "ATG - Checkout Smoke Test - #{device_store} - #{payment_method} - #{env}", js: true do
    ...
    context 'Pre-Condition: Register new account and claim devices' do
      create_account_and_claim_devices_via_webservice(ServicesInfo::CONST_CALLER_ID, first_name, last_name, email, password, location, platform)
    end
