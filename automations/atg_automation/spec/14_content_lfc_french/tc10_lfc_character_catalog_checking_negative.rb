...
=begin
French LFC Content: Verify Products that don't match the Character or Locale should not display
=end

describe "LFC French - Character Catalog Negative - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST}" do
  ...
  character_list.each do |ch|
    character_name = Title.french_to_english(ch['name'], 'character')
