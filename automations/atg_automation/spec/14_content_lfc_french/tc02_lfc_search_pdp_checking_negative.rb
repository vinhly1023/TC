...
=begin
French LFC Content: Verify that Apps that don't support for current Locale are not displayed
=end

describe "LFC French - Search negative checking - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST}" do
  context "Verify that Apps that don't support for current Locale are not displayed: #{AppCenterContent::CONST_SEARCH_URL}" do
    ...
    titles_list.each do |title|
      ...
      before :all do
        atg_app_center_page.load(AppCenterContent::CONST_SEARCH_FRENCH_URL % e_product[:prod_number])
