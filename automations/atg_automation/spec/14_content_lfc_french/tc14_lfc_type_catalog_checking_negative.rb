...
=begin
French LFC Content: Verify Products that don't match the Type/Format or Locale should not display
=end

describe "English LFC - Type/Format Catalog Negative - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST}" do
  ...
  type_list.each do |sk|
    type_name = Title.french_to_english(sk['name'], 'type')
    type_href = sk['href']
