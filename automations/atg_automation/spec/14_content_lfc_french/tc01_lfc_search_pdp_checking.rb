...
=begin
French LFC Content: Verify Product information on Search and PDP page
=end

describe "LFC French - Verify Product information on Search and PDP page - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST}" do
  ...
  titles_list.each do |title|
    ...
    context "#{tc_num += 1}. Product ID = '#{e_product_search[:prod_number]}' - Name = '#{e_product_search[:short_name]}'" do
      ...
      context 'Search and check Product information on Search page' do
        product_info = {}
        url = AppCenterContent::CONST_SEARCH_FRENCH_URL % e_product_search[:prod_number]
      ...
      context 'Product detail page checking' do
        ...
        it "Verify Notable/Highlights is '#{e_product_pdp[:highlights]}'" do
          expect(a_product_pdp[:notable]).to eq(e_product_pdp[:highlights])
        end
        ...
        it "Verify Credit (Credits) link exists: '#{e_product_pdp[:has_credit_link]}'" do
