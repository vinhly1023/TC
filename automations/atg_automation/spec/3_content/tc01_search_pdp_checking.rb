...
=begin
English Web Content: Verify Product information on Search and PDP page
=end

describe "Verify Product information on Search and PDP page - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST.upcase}" do
  ...
  titles_list.each do |title|
    ...
    context "#{tc_num += 1}. Product ID = '#{e_product_search[:prod_number]}' - Name = '#{e_product_search[:short_name]}'" do
      ...
      context 'Product Detail Checking' do
        ...
        it 'Go to PDP' do
          skip_flag = true
          pdp_url = a_product_search.empty? ? atg_app_center_page.generate_pdp_url(URL::PDP_WEB_URL, e_product_search[:long_name], e_product_search[:prod_number]) : AppCenterContent::CONT_PDP_URL % a_product_search[:href]
