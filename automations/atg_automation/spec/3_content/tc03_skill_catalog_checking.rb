...
=begin
English Web Content: Verify that Products that match with Skill and Locale should display
=end

describe "Skill catalog checking - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST.upcase}" do
  ...
  skills_list.each do |sk|
    ...
    context "TC#{tc_num += 1}: Skill = '#{skill_name}' - Total Apps = #{titles_count}" do
