...
=begin
English Web Content: Verify Products that don't match the Skill or Locale should not display
=end

describe "Skill Catalog Negative checking - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST.upcase}" do
  ...
  skills_list.each do |sk|
    ...
    context "TC#{tc_num += 1}: Skills = '#{skill_name}' - Total Apps = #{titles_count}" do
      next unless app_available?(titles_count, "There are no apps belong to Skill: #{skill_name}")
