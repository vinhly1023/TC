...
=begin
English Web Content: Verify user can search and check out app by using Account Balance
=end
...
if env != 'PREVIEW' && env != 'PROD'
  ...
  describe "Search SKU and Purchase App using exist balance checking - Env: #{env} - Locale: #{General::LOCALE_CONST}" do
      ...
      context "#{count += 1}. SKU = '#{e_product[:sku]}' - #{e_product[:short_name]}" do
        ...
        context "Purchase using exist balance to user '#{AppCenterAccount::BALANCE_ACC[0]}'" do
          it 'Check out process' do
            ...
            a_successful_msg = atg_checkout_page.checkout_with_account_balance
          end
