...
=begin
French Cabo Content: Verify Product information on Search and PDP page
=end

describe "CABO French - Verify Product information on Search and PDP page - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST.upcase}" do
  ...
  titles_list.each do |title|
    e_product_search = fr_cabo_atg_app_center_page.expected_catalog_product_info title
    e_product_pdp = fr_cabo_atg_app_center_page.expected_pdp_product_info title
    ...
    context "#{tc_num += 1}. Product ID = '#{e_product_search[:prod_number]}' - Name = '#{e_product_search[:short_name]}'" do
      ...
      context 'Search and check Product information on Search page' do
        ...
        it "Search by Product ID: #{url}" do
          ...
          product_info = fr_cabo_atg_app_center_page.catalog_product_info(product_html, e_product_search[:prod_number])
        ...
        it 'Get Product information on Search page' do
          a_product_search = fr_cabo_atg_app_center_page.actual_catalog_product_info product_info
        end
      ...
      context 'Product Detail Checking' do
        ...
        it 'Go to PDP' do
          skip_flag = true
          pdp_url = a_product_search.empty? ? fr_cabo_atg_app_center_page.generate_pdp_url(URL::PDP_CABO_URL, e_product_search[:long_name], e_product_search[:prod_number]) : AppCenterContent::CONT_PDP_URL % a_product_search[:href]
          fr_cabo_atg_app_center_page.navigate_to_pdp pdp_url
          ...
          skip_flag = false
          pdp_info = fr_cabo_atg_app_center_page.pdp_info
          a_product_pdp = fr_cabo_atg_app_center_page.actual_pdp_product_info pdp_info
        ...
        if e_product_pdp[:has_credit_link]
          it "Verify Credit (Crédits) text contains long name: '#{e_product_pdp[:credits_text]}'" do
            if General::ENV_CONST == 'PREVIEW'
              ...
              a_credits_app_title = fr_cabo_atg_app_center_page.credits_text
