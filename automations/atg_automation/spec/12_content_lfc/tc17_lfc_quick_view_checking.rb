...
=begin
English LFC Content: Open Quick View overlay and check app information
=end

describe "LFC English - Check information on Quick View page - Env: #{General::ENV_CONST} - Locale: #{General::LOCALE_CONST.upcase}" do
  ...
  titles_list.each do |title|
    ...
    context "#{count += 1}. Product ID = '#{e_product[:prod_number]}' - Name = '#{e_product[:short_name]}'" do
      ...
      it "Open Quick View dialog and get information if '#{e_product[:sku]}' is existed" do
        a_quick_view_info = atg_app_center_page.quick_view_info(e_product[:prod_number])
